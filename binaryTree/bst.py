...
def getPostorder(root):
    if root is None:
        return ""
    return str(getInorder(root.left)) + str(getInorder(root.right)) + str(root.val) + "-"
...
def getPreorder(root):
    if root is None:
        return ""
    return str(root.val) + "-" + str(getInorder(root.right)) + str(getInorder(root.left))
...
if __name__ == '__main__':
    ...
    root = insert(root, 8)
    print("Inorder traversal of binary tree with 8 inserted is")
    print(getInorder(root))
